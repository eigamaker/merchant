...
`build_game_assets.py` は最近傍補間を使用し、透過済み原画をセル内へ収めます。キャラクターについては、クロマキー除去後にセル外周へ残った微小な色を縮尺判定へ含めないよう、外周を除外してから不透明領域を測ります。旧自作環境素材は再生成対象から除外済みです。
...
| ダンジョン地形 | レガシー出力 | 再生成対象外。Craftpix素材への移行完了後に削除 |
