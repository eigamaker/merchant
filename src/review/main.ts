...
root.innerHTML = `
  <div class="map-editor-shell">
    ...
    <section class="palette-editor-view" data-editor-view="palette" hidden><div class="palette-editor-toolbar"><span>左の素材を選択し、右のパレットへドラッグして配置します。素材の種類とセルサイズは選択中のパレットに自動で合います。</span></div><div class="palette-workspace"><section class="asset-folder-panel" data-asset-folder><div class="panel-heading"><div><h2>素材フォルダ（Project）</h2><p class="small">フォルダを選び、タイルをパレットへドラッグします。</p></div><span data-asset-folder-status></span></div><select class="visually-hidden" data-asset aria-label="素材シート"></select><div class="asset-browser"><nav class="asset-image-picker" data-asset-picker aria-label="素材フォルダ"></nav><div class="asset-content"><div class="source-wrap asset-source-wrap"><canvas data-source-canvas></canvas></div><p class="small">方眼で単体または矩形を選択し、そのまま右のパレットへドラッグできます。</p></div></div></section><div class="palette-host" data-palette-host="palette"></div></div></section>
...
const palettePanel = q<HTMLElement>(".palette-panel"), paletteSaveButton = q<HTMLButtonElement>("[data-action=palette-save]");
...
function selectPageForEditor(): void { if (editorMode !== "palette") return selectPageForMap(); if (!activePage() && active) activePageId = selectCompatiblePalettePageId(palettes.layout.pages, activePageId, active.tileSize); paletteRect = undefined; }
function ensurePalettePage(): PalettePage | undefined { if (!active) return undefined; let page = pageForEditor(); if (page) return page; const id = uniqueId(`${active.kind}-${active.tileSize}`); palettes.mutate((layout) => addPalettePage(layout, createPalettePage({ id, label: `${active!.kind === "home" ? "家" : "ダンジョン"} ${active!.tileSize}px`, mapKind: active!.kind, tileSize: active!.tileSize, width: 2048, height: 2048 }))); activePageId = id; page = activePage(); return page; }
...
function renderPaletteTabs(): void {
  if (!active) return;
  const compatible = editorMode === "palette" ? palettes.layout.pages : palettes.layout.pages.filter((page) => page.tileSize === active!.tileSize);
  paletteTabs.innerHTML = compatible.map((page) => `<button type="button" role="option" aria-selected="${page.id === activePageId}" data-page-id="${escapeHtml(page.id)}" class="palette-page-card${page.id === activePageId ? " active" : ""}"><span>${escapeHtml(page.label)}</span><small>${page.width}×${page.height}</small></button>`).join("") || `<p class="small">${active.tileSize}px のページがありません。</p>`;
...
function renderAssetOptions(): void {
  if (!active) return;
  const contextKind = editorMode === "palette" ? activePage()?.mapKind : active.kind;
  const contextTileSize = editorMode === "palette" ? activePage()?.tileSize : active.tileSize;
  const allowed = contextKind && contextTileSize ? assets.filter((asset) => asset.mapKinds.includes(contextKind) && asset.tileSize === contextTileSize) : [];
...
function renderMapActorStrip(): void {
  ...
  paletteActorStripList.innerHTML = actors.map((actor) => {
    ...
    return `<label class="actor-strip-card${selected ? " selected" : ""}">${control}<canvas data-strip-actor-thumb="${escapeHtml(actor.id)}" width="58" height="58"></canvas><span>${escapeHtml(actor.label)}</span><small>HP ${actor.enemyStats?.baseHp ?? "?"}</small>${remove}</label>`;
...
function renderImportReport(): void {
  ...
  q<HTMLButtonElement>("[data-action=import-commit]").onclick = async () => {
    ...
    try {
      const mapTiles = (analysis.mapTiles ?? []).map((entry: any) => { const selected = importReport.querySelector<HTMLInputElement>(`[data-import-map-id="${CSS.escape(entry.id)}"]`)?.checked ?? false, tileValue = importReport.querySelector<HTMLSelectElement>(`[data-import-tile-size="${CSS.escape(entry.id)}"]`)?.value ?? "", tileSize = tileValue ? Number(tileValue) : undefined, margin = Number(importReport.querySelector<HTMLInputElement>(`[data-import-margin="${CSS.escape(entry.id)}"]`)?.value ?? 0), spacing = Number(importReport.querySelector<HTMLInputElement>(`[data-import-spacing="${CSS.escape(entry.id)}"]`)?.value ?? 0); if (selected && !tileSize) throw new Error(`${entry.label}: 16pxまたは32pxのセルサイズを選択してください`); return { ...entry, selected, tileSize, margin, spacing }; });
      ...
      const response = await fetch(`/__map-editor/import/${pendingImport!.id}/commit`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mapTiles, actors, createPalettePages: importReport.querySelector<HTMLInputElement>("[data-import-pages]")?.checked === true, licenseAcknowledged }) }); if (!response.ok) throw new Error(await response.text()); status.textContent = "素材を登録しました。再読込します。"; window.setTimeout(() => window.location.reload(), 250);
    } catch (error) { status.textContent = `素材登録エラー: ${error instanceof Error ? error.message : "不明"}`; }
  };
}
...
const addPalette = () => { if (!active) return; const context = editorMode === "palette" ? activePage() : undefined, id = uniqueId("palette"), pageKind = context?.mapKind ?? active.kind, pageTileSize = context?.tileSize ?? active.tileSize; palettes.mutate((layout) => addPalettePage(layout, createPalettePage({ id, label: "新しいパレット", mapKind: pageKind, tileSize: pageTileSize, width: 2048, height: 2048 }))); activePageId = id; paletteRect = undefined; render(); };
...
void (async () => { maps = await repository.list(); if (!maps.length) { const defaults = createDefaultMapPack(); maps = [defaults.home, ...defaults.dungeons]; for (const map of maps) await repository.save(map); } await loadActorSettings(); setActive(maps.find((map) => map.kind === "home") ?? maps[0]!); await loadPalettes(); window.requestAnimationFrame(animateActorPreview); })();
