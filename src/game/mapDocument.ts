...
export interface MapDocument {
  version: 6; id: string; name: string; kind: MapKind; floor: number; width: number; height: number; tileSize: TileSize;
  /** Retained for legacy tools; v6 rendering reads the explicit layers. */ terrain: Array<string | null>;
  /** true means walkable.  This is mandatory in v6. */ collision: boolean[];
  layers: Record<ManualLayer, Array<ManualTilePlacement | null>>; markers: MapMarker[];
  /** Enemy actors selected for this dungeon floor; positions remain random at runtime. */ enemyRoster: string[];
...
export function createBlankMap(first: MapKind | string, second?: string | MapCreateOptions, third?: MapCreateOptions): MapDocument {
  ...
  return { version: 6, id: `map-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`, name, kind, floor: Math.max(0, Math.floor(options.floor ?? (kind === "home" ? 0 : 1))), width, height, tileSize, terrain: Array(size).fill(null), collision: Array(size).fill(false), layers: { ground: Array(size).fill(null), structure: Array(size).fill(null), decoration: Array(size).fill(null) }, markers: [], enemyRoster: [], createdAt: now, updatedAt: now };
}
export const createManualMap = createBlankMap;
export function cloneMap(map: MapDocument): MapDocument { return { ...map, terrain: [...map.terrain], collision: [...map.collision], layers: Object.fromEntries(LAYERS.map((layer) => [layer, map.layers[layer].map((cell) => cell ? { ...cell } : null)])) as MapDocument["layers"], markers: map.markers.map((marker) => ({ ...marker, visual: marker.visual ? { ...marker.visual } : undefined })), enemyRoster: [...(map.enemyRoster ?? [])] }; }
...
function isEmpty(map: MapDocument): boolean { return map.terrain.every((value) => value === null) && map.collision.every((value) => !value) && LAYERS.every((layer) => map.layers[layer].every((value) => value === null)) && map.markers.length === 0 && map.enemyRoster.length === 0; }
...
export function validateStructure(value: unknown): string[] {
  ...
  if (map.version !== 6) errors.push("version"); if (map.kind !== "home" && map.kind !== "dungeon") errors.push("kind"); if (!Number.isInteger(map.width) || !Number.isInteger(map.height) || map.width! < MAP_SIZE_MIN || map.height! < MAP_SIZE_MIN || map.width! > MAP_SIZE_MAX || map.height! > MAP_SIZE_MAX) errors.push("dimensions"); if (!Number.isInteger(map.floor) || map.floor! < 0) errors.push("floor"); if (map.tileSize !== 16 && map.tileSize !== 32) errors.push("tileSize");
...
export function validateMap(map: MapDocument): string[] {
  const errors = validateStructure(map); if (errors.length) return errors; if (map.kind === "home" && map.enemyRoster.length) errors.push("home enemy roster"); const allowed = map.kind === "home" ? ["homeSpawn", "dungeonEntrance", "homeStorage", "homePreparation", "homeVisitors", "shopkeeperCounter", "customerCounter"] as const : ["stairsUp", "stairsDown"] as const, required = map.kind === "home" ? ["homeSpawn", "dungeonEntrance", "homePreparation", "homeVisitors"] as const : ["stairsUp", "stairsDown"] as const, ids = new Set<string>();
...
type LegacyMap = { version: 3 | 4 | 5; id: string; name: string; kind: MapKind; floor: number; width: number; height: number; tileSize?: number; terrain: Array<string | null>; collision?: boolean[]; tileFrames?: Array<number | null>; layers?: Partial<Record<ManualLayer, Array<{ tileId?: string; assetId?: string; frame: number } | null>>>; markers: Array<{ id: string; kind: string; x: number; y: number; visual?: MapMarkerVisual }>; enemyRoster?: string[]; createdAt: string; updatedAt: string; };
...
const DB = "home-dungeon-map-editor", STORE = "maps", TRIAL = "home-dungeon-map-trial:v6", LEGACY_TRIAL = "home-dungeon-map-trial:v3", LEGACY_TRIAL_V5 = "home-dungeon-map-trial:v5";
