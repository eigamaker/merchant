...
export function normalizeMap(value: unknown): MapDocument { if (!value || typeof value !== "object") throw new Error("invalid MapDocument v3/v4/v5/v6: document object"); const raw = value as { version?: number }; const map = raw.version === 3 || raw.version === 4 || raw.version === 5 ? migrateLegacy(value as LegacyMap) : cloneMap(value as MapDocument); const errors = validateStructure(map); if (errors.length) throw new Error(`invalid MapDocument v3/v4/v5/v6: ${errors.join(", ")}`); return map; }
